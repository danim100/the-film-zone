...
SOURCES = [{"url": "https://goo.gl/MX5nfk", "fanart": "http://i.imgur.com/q2mlY5D.jpg"}]

def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.metropolitan Lists-%s]: %s" %(addon_version, string)) #    modificar("el nombre de tu addons")
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(metropolitan,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(metropolitan,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
